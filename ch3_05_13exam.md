...
**Q2.** `[Conceptual]` Why do Large Language Models like Claude require mechanisms such as CLAUDE.md for persistent context?
...
**Q3.** `[Applied]` A developer notices that within a single session, Claude Code seems to remember earlier messages. But when they close the terminal and start a new session, Claude remembers nothing. What explains this behavior?

- A) Claude Code saves conversation logs locally but clears them on terminal close for security reasons
- B) Claude stores session memory in RAM that is flushed when the process exits, requiring CLAUDE.md for persistence
- C) Within a session, Claude Code re-sends the entire message history with each call — creating the illusion of continuity — but the LLM itself retains nothing between sessions
- D) Claude Code maintains a local SQLite database of conversations that gets corrupted when the terminal closes unexpectedly
...
**Q4.** `[Direct]` Which six sections does a typical CLAUDE.md contain?
...
**Q5.** `[Applied]` A team lead creates a CLAUDE.md file but places it inside `src/docs/CLAUDE.md`. New Claude Code sessions don't pick it up. A teammate suggests the file might also be named `claude.md` in some editors. What is the most likely root cause?

- A) The file must be named in lowercase as `claude.md` for cross-platform compatibility
- B) CLAUDE.md must be encoded in UTF-8 without BOM or Claude Code ignores it
- C) Claude Code only auto-detects CLAUDE.md at the project root — the same level as `.git` — not in subdirectories
- D) The file exceeds the 3 KB size limit and Claude Code silently skips oversized context files
...
**Q6.** `[Conceptual]` What is the core insight behind Claude Code using the file system as external memory instead of trying to keep everything in conversation history?
...
**Q7.** `[Evaluative]` A senior developer argues: "CLAUDE.md is redundant — I can just paste my project context at the start of every session and get the same results." A junior developer disagrees. What is the strongest counter-argument?

- A) CLAUDE.md is encrypted and more secure than pasting context into a conversation prompt
- B) Pasting context manually introduces human variability each time, while CLAUDE.md ensures consistent, complete context is loaded automatically — and it also enables team alignment since everyone shares the same project brief
- C) Claude Code's API rejects prompts longer than 500 tokens, making manual pasting infeasible for complex projects
- D) CLAUDE.md loads before the system prompt, giving it higher priority than user-supplied context in the conversation
...
**Q9.** `[Conceptual]` What is the recommended relationship between CLAUDE.md and AGENTS.md in the same project?

- A) They should contain identical content to ensure consistency across all agents
- B) AGENTS.md replaces CLAUDE.md entirely since it's the universal standard
...
- D) CLAUDE.md holds all project context; AGENTS.md is only used when Claude Code is unavailable
...
**Q10.** `[Applied]` A company uses Claude Code, Cursor, and GitHub Copilot across different teams. They currently maintain separate context files for each tool. An architect proposes consolidating to AGENTS.md plus tool-specific files. What is the primary benefit?

- A) It reduces the total number of files in the repository from three to two
- B) Universal project context is written once in AGENTS.md and works across all three tools, eliminating duplication while each tool-specific file adds only its unique features
- C) AGENTS.md is the only format that supports YAML frontmatter for structured metadata
- D) Consolidation forces all teams to standardize on a single AI coding tool eventually
...
**Q11.** `[Direct]` What does "non-deterministic" mean in the context of AI language models?
...
**Q12.** `[Conceptual]` The "double variability problem" compounds unpredictability in AI outputs. What are the two sources of drift?
...
**Q13.** `[Conceptual]` How do skills constrain non-determinism without eliminating it?
...
**Q14.** `[Applied]` A content creator runs the same prompt — "Write a LinkedIn post about learning AI development" — twice without any skill. They get different results each time: different structure, different emoji count, different tone. They then create a skill specifying "friendly-professional tone, 2-3 emojis, end with engagement question." What changes?

- A) Both runs produce byte-identical output since the skill eliminates all randomness from generation
- B) Both runs still differ in wording and details, but both maintain the friendly-professional tone, use 2-3 emojis, and end with an engagement question
- C) The skill forces Claude to select from a predefined library of pre-written LinkedIn templates
- D) The first run uses the skill but the second run reverts to default behavior unless the skill is re-invoked
...
**Q15.** `[Conceptual]` The formula "Intelligence + Code = Execution, but not Expertise" captures a key insight. What is the missing piece that skills provide?

- A) Additional computational resources for faster model inference
- B) Domain-specific knowledge that makes generic capability specifically useful for real work
- C) Network connectivity to external APIs and databases
- D) Version control integration for tracking changes over time
...
**Q16.** `[Direct]` In the three-level loading architecture for skills, what is loaded at each stage?
...
**Q17.** `[Applied]` A developer has 50 skills installed and worries about overwhelming Claude's context window. Based on the three-level loading architecture, why is this concern unfounded?

- A) Claude Code compresses all 50 skills into a single token-efficient binary blob at startup
- B) Skills are loaded alphabetically and Claude stops after the first 10 to preserve context space
- C) Only brief descriptions are loaded at startup — full instructions load on-demand only when a skill is relevant, like phone apps that stay closed until you tap them
- D) Claude Code offloads skill storage to a cloud cache that doesn't consume local context tokens
...
**Q18.** `[Evaluative]` A CTO claims: "We should build a custom AI agent for our legal department — they need specialized contract review capabilities." Based on the "stop building agents, build skills instead" paradigm, what is the strongest counter-argument?

- A) Custom agents are technically impossible to build with current LLM frameworks
- B) The agent infrastructure already exists in Claude Code — what's missing is the legal team's contract review expertise encoded as skills, which is simpler to create, maintain, and share than building an entire agent
- C) Skills are limited to text processing and cannot handle the document formats used in legal work
- D) Custom agents would require Anthropic's enterprise license, while skills are available on all plans
...
**Q19.** `[Direct]` What are the three sources from which skills emerge?
...
**Q20.** `[Conceptual]` How do skills and MCP servers complement each other rather than compete?
...
**Q21.** `[Applied]` A financial services team has an MCP server connected to their accounting database. Claude can query data but produces generic, unstructured reports. What would adding a skill accomplish?

- A) The skill would replace the MCP connection and access the database directly through local scripts
- B) The skill would encode the team's reporting procedures — what reports to generate, what format to use, what insights to highlight — so Claude produces reports matching organizational expectations
- C) The skill would cache all database results locally to eliminate MCP latency on subsequent queries
- D) The skill would add authentication layers that MCP servers cannot provide on their own
...
**Q22.** `[Conceptual]` Why is the "skills are applications" analogy (Models = Processors, Agent Runtimes = OS, Skills = Applications) significant for adoption?

- A) It means only large companies with processor-level resources can create meaningful skills
- B) It positions skills as the layer where anyone can contribute domain expertise — just as millions build apps without building processors or operating systems
- C) It implies that skills must be compiled into binary formats like traditional applications
- D) It suggests that skills will eventually replace both models and agent runtimes entirely
...
**Q23.** `[Evaluative]` A recruiter with no programming experience hesitates: "Skills sound like a developer thing — I can't create them." Based on Anthropic's early observations, what is the most compelling response?

- A) Recruiters can learn Python in a weekend course and then build skills with scripting support
- B) In the first weeks after launch, skills were being built by non-technical people in finance, recruiting, accounting, and legal — because skills require clear instructions in markdown, not coding ability
- C) Anthropic provides a marketplace where recruiters can purchase pre-built skills for their domain
- D) The skill-creator meta-skill writes all the code automatically so the recruiter never sees technical details
...
**Q24.** `[Direct]` What are the two required parts of a SKILL.md file?
...
**Q25.** `[Applied]` A developer writes this skill description: `description: "Helps with notes"`. The skill rarely activates when they share meeting content. What is wrong and how should it be fixed?

- A) The description needs to include the skill's version number and author name for Claude to index it properly
- B) The description is too vague — "notes" could mean anything — it should specify what it does, key outputs, and trigger conditions, like: "Transform meeting transcripts into structured summaries with action items, decisions, and follow-ups. Use when user shares meeting content."
- C) The description must be written as a Python regular expression so Claude can pattern-match against user input
- D) The description should list every possible user phrase that could trigger activation for complete coverage
...
**Q26.** `[Conceptual]` Why is the description field called "the most important line" in a SKILL.md file?
...
**Q27.** `[Applied]` A developer creates a blog-planner skill and tests it. The output doesn't include SEO considerations or word count targets. They ask Claude to review the skill and suggest improvements. Claude recommends adding those sections. The developer then adds their own constraint: "headlines must be curiosity-driven, never clickbait." What pattern does this interaction illustrate?

- A) The waterfall development pattern where requirements are gathered before implementation begins
- B) The co-learning refinement cycle — Claude teaches improvements the developer didn't consider, the developer teaches domain constraints Claude doesn't know, and they converge on the right solution
- C) The automated testing pattern where Claude validates skill output against acceptance criteria
- D) The version control pattern where each skill revision is committed with a descriptive changelog
...
**Q28.** `[Direct]` Where do skill folders live in a Claude Code project?
...
**Q29.** `[Applied]` A team member uses the skill-creator meta-skill to generate a meeting-notes skill by describing their procedure. Meanwhile, another colleague manually writes a code-review skill from scratch following the SKILL.md anatomy. Both approaches produce working skills. When should each approach be preferred?

- A) The skill-creator should always be preferred because it produces higher-quality SKILL.md files than manual creation
- B) Manual creation is only needed when the skill-creator is not installed in the project
- C) The skill-creator is best for most people to get started quickly; manual creation is useful for understanding the internals and fine-tuning skills that need precise control
- D) The skill-creator produces draft skills that must always be manually rewritten before use
...
**Q30.** `[Conceptual]` What is the key difference between a saved prompt and a skill?
...
**Q31.** `[Direct]` What are the two activation modes for skills?
...
**Q32.** `[Evaluative]` An engineering manager asks: "Why should we invest time encoding our team's procedures as skills when we can just copy-paste our prompts?" Compare manual prompting to agent skills across reliability, token cost, and reusability to form the strongest response.

- A) Skills are more visually appealing in the codebase and easier to discover through file browsing than scattered prompt files
- B) Manual prompts are ad-hoc and pay for rules in every conversation; skills are deterministic, load rules only when triggered, and become reusable IP that can be shared with the team, versioned in Git, and integrated into custom agents
- C) Skills execute faster because they bypass the LLM entirely and run as compiled scripts
- D) Manual prompts consume zero tokens while skills always add overhead, but skills provide better formatting
...
**Q33.** `[Direct]` What is a subagent in Claude Code?
...
**Q34.** `[Conceptual]` Why do subagents use isolated context windows instead of sharing the main conversation context?

- A) Isolated contexts allow subagents to use different AI models than the main session
- B) Sharing context would violate data privacy regulations across different task domains
- C) Isolation prevents context clutter — each specialist focuses on one job without being confused by unrelated conversation history, producing more focused results
- D) The API only supports one active context window, so subagents must queue their requests
...
**Q35.** `[Applied]` A developer asks Claude Code: "Find all test files in this project AND suggest a testing strategy for gaps." Claude Code launches both the Explore and Plan subagents simultaneously. What orchestration pattern is this demonstrating?

- A) Sequential delegation where Plan waits for Explore to finish before starting
- B) Parallel agent execution — multiple specialists working simultaneously with isolated contexts, combining results into a single response
- C) Recursive delegation where Explore spawns Plan as a child process
- D) Load-balanced execution where the faster subagent handles both tasks
...
**Q36.** `[Applied]` A developer creates a custom subagent called `code-reviewer` using the `/agents` menu. They test it with "Use the code-reviewer subagent to review this function." The review completes. The developer then asks a follow-up: "Can you also check the security aspects?" Who handles this follow-up?

- A) The code-reviewer subagent, which maintains its context and continues the review
- B) Main Claude Code, because subagents complete their task and return control — they don't persist for follow-up dialogue
- C) A new instance of code-reviewer that inherits the previous instance's findings
- D) The built-in Plan subagent, which automatically takes over for multi-step tasks
...
**Q37.** `[Conceptual]` Where do custom subagent configuration files live, and what determines their scope?
...
**Q38.** `[Applied]` A team needs to decide between creating a skill and creating a subagent for their code comment formatting standards. The task is lightweight, happens frequently, and should trigger automatically when Claude writes code. Which should they choose and why?

- A) A subagent, because subagents provide guaranteed execution through hard invocation
- B) A skill, because it's lightweight, supports automatic activation, and shares the main conversation context — ideal for repeated patterns like formatting
- C) Both a skill and a subagent working in tandem for redundancy
...
**Q39.** `[Direct]` What does MCP stand for, and what problem does it solve?
...
**Q40.** `[Applied]` A developer needs Claude to check the latest React documentation for a new hook API. Without MCP, Claude would rely on training data that might be outdated. With Context7 MCP installed, what changes?
...
**Q41.** `[Conceptual]` The "phone directory" mental model describes MCP's design. How does this analogy apply?

- A) MCP stores a list of frequently called API endpoints sorted by usage frequency
- B) MCP servers are approved specialists Claude can call — a web browser expert, a documentation specialist, a database consultant — each handling one external capability through secure channels
- C) MCP acts as a phone switchboard routing Claude's requests to the cheapest available API provider
- D) MCP maintains a directory of Claude Code users for team collaboration features
...
**Q42.** `[Applied]` A developer runs `claude mcp add --transport stdio playwright npx @playwright/mcp@latest` and then asks Claude to browse Amazon for products. Claude launches Playwright, navigates pages, extracts details, and returns a summary. The developer then says "filter to long-sleeve only." What makes this iterative refinement possible?

- A) Playwright MCP caches the previous search results locally and applies client-side filtering
- B) Claude maintains the MCP session context and can issue follow-up navigation commands to refine results naturally within the same conversation
- C) The developer must re-run the entire search with the new filter because MCP doesn't support iteration
- D) Amazon's API provides a filter parameter that Playwright passes directly without re-browsing
...
**Q43.** `[Evaluative]` A junior developer proposes: "Let's install this cool MCP server I found on a random npm package to access our production database." Based on MCP security principles, what concerns should be raised?

- A) The only concern is whether the npm package is compatible with the team's Node.js version
- B) Untrusted MCP servers can access the internet, read files, and expose tokens — only servers from reputable sources (Anthropic's official list, modelcontextprotocol.io, verified npm packages) should be installed, and the source code should be reviewed on GitHub first
- C) MCP servers are sandboxed and cannot access anything beyond their declared permissions regardless of their source
- D) The concern is valid only for production environments; development environments are inherently safe for any MCP server
...
**Q44.** `[Direct]` What does MCP Tool Search do, and when does it activate?
...
**Q45.** `[Applied]` A developer has 5 MCP servers installed, consuming an estimated 25,000-40,000 tokens of tool definitions before asking a single question. After updating to Claude Code 2.1.7+, they notice Tool Search activates and overhead drops significantly. However, for their complex daily browser automation workflow involving 10+ operations, they still want better efficiency. What should they consider?

- A) Uninstalling 3 of the 5 MCP servers to reduce the base token footprint below the Tool Search threshold
- B) Compiling the browser automation MCP into a skill — where SKILL.md loads ~150 tokens and scripts execute locally at 0 context tokens — achieving ~98% reduction beyond Tool Search's ~85%
- C) Increasing the Tool Search threshold to 20% so it activates less aggressively and loads more tools upfront
- D) Switching to a model with a larger context window to accommodate the full tool definitions
...
**Q46.** `[Conceptual]` Why does direct MCP usage cause "token bloat," and how does the code execution pattern solve it?

- A) MCP servers send data in uncompressed XML format; compiled skills use compressed JSON
- B) Direct MCP loads all tool definitions into Claude's context (~5,000-8,000 tokens per server); compiled skills execute scripts locally outside context, so only filtered results (~150-200 tokens) enter the conversation
...
**Q48.** `[Applied]` The fetch-library-docs skill offers `--content-type` flags: `setup`, `examples`, and `api-ref`. A developer asks for Next.js installation instructions using `--content-type setup`. Instead of receiving the full documentation page (~800 tokens), they get only terminal commands and setup steps (~100 tokens). What design principle enables this?

- A) The LLM summarizes the full documentation before returning it to conserve tokens
- B) Content-type filtering happens locally in shell scripts — extracting only the requested category before results enter Claude's context
- C) Context7 MCP natively supports content-type parameters and filters at the server level
- D) The skill maintains a local cache of pre-filtered documentation for each content type
...
**Q49.** `[Conceptual]` The three-stage progressive disclosure model for compiled skills is: Discovery, Activation, Execution. What happens at each stage, and why is Stage 3 critical for token savings?

- A) Discovery indexes skills, Activation validates permissions, Execution calls the MCP API — Stage 3 matters because API calls are batched
- B) Discovery loads the description (~30 tokens), Activation loads full SKILL.md (~150 tokens), Execution runs scripts locally (0 tokens in context) — Stage 3 is critical because heavy MCP operations consume zero tokens
- C) Discovery caches tool schemas, Activation compiles scripts, Execution deploys to a container — Stage 3 matters because containers isolate resource usage
- D) Discovery scans the file system, Activation loads dependencies, Execution spawns a subagent — Stage 3 matters because subagents have separate token budgets
...
**Q50.** `[Applied]` A team lead is deciding which of their four MCP servers to compile into skills. Server A has 1,200 tokens of definitions and is used once monthly. Server B has 8,000 tokens, is used 5 times daily, and returns large datasets needing filtering. Server C has 2,000 tokens with a rapidly changing API. Server D has 6,000 tokens and is used in multi-step team workflows shared via Git. Which servers should be compiled?

- A) All four servers should be compiled for maximum token efficiency across the board
- B) Servers B and D — B has high overhead, frequent use, and needs filtering; D has high overhead and is a shared team workflow. A is too infrequent and C's rapidly changing API would require constant skill maintenance
- C) Only Server B because it has the highest token count of all four servers
...
**Q51.** `[Evaluative]` The "Three Pillars" framework states: CLAUDE.md gives Claude context, Skills give Claude procedures, and MCP gives Claude reach. A team has implemented CLAUDE.md and MCP but has no skills. They find Claude can access all external data but produces generic, inconsistent reports. Which pillar is missing and what is the consequence?

- A) The context pillar is missing — Claude doesn't know the project structure and cannot locate relevant data sources
- B) The reach pillar is missing — Claude cannot access the external databases needed for report generation
- C) The procedures pillar (Skills) is missing — Claude has context and data access but no encoded expertise for how to analyze, format, and present reports according to the team's standards
- D) All three pillars are present but misconfigured — the issue is a CLAUDE.md syntax error preventing proper loading
...
**Q52.** `[Evaluative]` A startup adopts the full stack: CLAUDE.md for project context, skills for domain procedures, MCP for external data, and compiled skills for token efficiency. A new engineer joins and starts a Claude Code session on day one. Compared to a company using none of these, what is the compounding organizational advantage?

- A) The new engineer saves approximately 30 minutes per day in reduced typing due to auto-complete features
- B) Claude already knows the project context, the team's procedures, and can access external systems — the new engineer gets an AI assistant on day one that embodies the organization's accumulated expertise, with zero ramp-up period
- C) The startup pays lower API costs due to compiled skills, which is the primary financial advantage over competitors
- D) The new engineer can bypass code reviews since Claude enforces all quality standards automatically
...
Direct:              10 (19.2%)
Conceptual:          13 (25.0%)
Applied Scenario:    21 (40.4%)
Evaluative Scenario:  8 (15.4%)
```
